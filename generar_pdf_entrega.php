...
// Propósito: Generar PDF público con validación QR real

// 1. Configuración de Errores y Buffering
...
if (!file_exists('fpdf/fpdf.php')) {
    die("Error crítico: Falta la carpeta 'fpdf'.");
}
require 'fpdf/fpdf.php';

session_start();
ob_end_clean(); // Limpiar cualquier salida previa
ob_start();     // Iniciar buffer limpio para PDF
...
// 3. Preparar Rutas y URLs Públicas
...
// Calcular URL Pública (Dinámica)
$protocol = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http";
$host = $_SERVER['HTTP_HOST'];
$path = dirname($_SERVER['PHP_SELF']);
// Asegurar que el path no termine en barra duplicada
$path = rtrim($path, '/');
...
class PDF extends FPDF {
    ...
    function Footer() {
        global $url_publica_pdf; // Acceder a la URL pública
        $this->SetY(-35);
        ...
        // Apunta directamente al archivo PDF en el servidor
        $qr_api = "https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=" . urlencode($url_publica_pdf);
        
        // Carpeta temporal para descargar el QR
        $temp_qr = 'uploads/temp_qr_' . uniqid() . '.png';
...
$pdf->Cell($w[1], 8, texto('DESCRIPCIÓN DEL INSUMO'), 0, 0, 'L', true);
...
foreach ($items as $item) {
    ...
    $pdf->Cell($w[1], 7, texto($item['nombre_insumo']), 'LR', 0, 'L', $fill);
...
// 4. Salida: GUARDAR EN DISCO Y LUEGO MOSTRAR
// 'F' guarda el archivo localmente
$pdf->Output('F', $ruta_fisica_archivo);

// 'I' envía el archivo al navegador
ob_end_clean(); // Limpiar buffer final
