...
// Propósito: Vista de Orden de Compra Médica (Diseño Profesional)
...
// LÓGICA DE APROBACIÓN (Director o Admin)
    ...
    if (in_array('Director Médico', $roles_usuario) || in_array('Administrador', $roles_usuario)) {
        try {
            ...
            $pdo->prepare("INSERT INTO notificaciones (id_usuario_destino, mensaje, url_destino) VALUES (?, ?, ?)")
                ->execute([$datosOC['id_usuario_creador'], $msj_creador, "insumos_oc_ver.php?id=" . $id_oc]);

            // Si se aprueba, notificar al Depósito Insumos
            if ($nuevo_estado == 'aprobada_logistica') {
                $rolDeposito = obtenerIdRolPorPermiso('recibir_oc_insumos'); // Usamos la función global
                if ($rolDeposito) {
                    $pdo->prepare("INSERT INTO notificaciones (id_rol_destino, mensaje, url_destino) VALUES (?, ?, ?)")
                        ->execute([$rolDeposito, "Autorizado por Dirección Médica. OC #{$datosOC['numero_oc']}", "insumos_recepcion.php?id=" . $id_oc]);
                }
            }
        ...
        $mensaje = '<div class="alert alert-danger shadow-sm border-0 mb-4">⛔ Solo el Director Médico puede autorizar.</div>';
    }
}
...
<div class="container-fluid px-4 mb-5">
    <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Gestión de Insumos Médicos</p>
        </div>
        <div>
            <a href="insumos_compras.php" class="btn btn-outline-secondary me-2"><i class="fas fa-arrow-left me-1"></i> Volver</a>
    ...
    <div class="card shadow-sm border-0">
        <div class="card-header <?php echo $estado_color; ?> text-white d-flex justify-content-between align-items-center py-3">
            ...
            <?php if (($orden['estado'] == 'aprobada_logistica' || $orden['estado'] == 'recibida_parcial') && (in_array('Encargado Depósito Insumos', $roles_usuario) || in_array('Administrador', $roles_usuario))): ?>
                <a href="insumos_recepcion.php?id=<?php echo $orden['id']; ?>" class="btn btn-light btn-sm text-primary fw-bold">Ir a Recepción <i class="fas fa-arrow-right ms-1"></i></a>
            <?php endif; ?>
        </div>
        ...
        <div class="card-body p-4">
            <div class="row mb-4">
                <div class="col-md-6">
                    <h5 class="text-primary fw-bold text-uppercase mb-3">Policlínica ACTIS</h5>
                ...
                <table class="table table-bordered table-striped align-middle">
                    ...
                    <tbody>
                        ...
                        foreach ($items as $item): 
                            ...
                            $servicio_solicitante = $orden['servicio_destino']; // Default si no tiene corchetes
                            
                            // Regex para sacar texto entre corchetes si existe
                    ...
                    <tfoot class="bg-light">
                        <tr>
                            ...
                            <td class="text-end fw-bold fs-5 text-primary pt-3">$ <?php echo number_format($grand_total, 2); ?></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
